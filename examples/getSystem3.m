function [f, g, h, zInit, M] = getSystem3(n, m, p, epsilon, alpha)
%getSystem3  An input-output system based on discretization of the Burgers eqn.
%
%  Usage: [f,g,h,zInit] = getSystem3(n,m,p,epsilon,alpha)
%
...
%
%  Produces a finite element model of (using linear elements)
%
%       z_t = -zz_x + epsilon z_xx + alpha z + B u,
%         y = C z
%                                                  { 0.5*sin(2*pi*x).^2, 0<x<0.5
%  on domain (0,1), zero Dirichlet BC, &  z(0,x) = {
%                                                  { 0                 , 0.5<x<1
%
%  g = {B} represents (m) equispaced piecewise-constant source terms, and
%  h = {C} represents integral averages over (p) equispaced portions of the domain.
%
...
[M, A, B, C, N, zInit] = BurgersFEMControl(n, m, p);
...
[x, e_conn] = oned_mesh([0; 1], [1 2], n + 1); % build a linear mesh on (0,1)

[n_nodes, ~] = size(x);
[n_elements, nel_dof] = size(e_conn);

ide = 1:n_nodes; % setting equation numbers
...
A = A(2:n_equations - 1, 2:n_equations - 1);
M = M(2:n_equations - 1, 2:n_equations - 1);

B = B(2:n_equations - 1, :);
C = C(:, 2:n_equations - 1);

N = zeros(n_equations - 2, (n_equations - 2) * (n_equations - 2));
for i = 2:n_equations - 1
    tmp = NN(2:n_equations - 1, 2:n_equations - 1, i)';
    N(i - 1, :) = tmp(:)';
end

zInit = M \ z0(2:n_equations - 1);
end
