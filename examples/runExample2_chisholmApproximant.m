...
for i = 1:nY
    for j = 1:nX
        ...
        % wRES(i, j) = computeResidualFutureHJB_2D_example2(gravity, L, g, h2q(h), 0, w, degree, x);
        if eFuture(i, j) < 0
            eFuture(i, j) = NaN;
        end
    end
...
if exportPlotData
    ...
    exportgraphics(fig4, sprintf('plots/example2_Chisholm_futureEnergy_m%i_n%i.pdf', M, N), 'ContentType', 'vector', 'BackgroundColor', 'none');
end
...
% function [res] = computeResidualFutureHJB_2D_example2(gravity, L, g, h, eta, w, degree, x)
%
% w = w(1:degree);
%
% %         constant B input
